...
class Loss(torch.nn.Module):
    ...
    def get_targets(y_true: torch.Tensor, anchors: torch.Tensor) -> torch.Tensor:
        targets_dx = (y_true[..., 0] - anchors[..., 0]) / anchors[..., 3]
        targets_dy = (y_true[..., 1] - anchors[..., 1]) / anchors[..., 4]
        targets_dz = (y_true[..., 2] - anchors[..., 2]) / anchors[..., 5]
        targets_dl = torch.log(y_true[..., 3] / anchors[..., 3])
        targets_dw = torch.log(y_true[..., 4] / anchors[..., 4])
        targets_dh = torch.log(y_true[..., 5] / anchors[..., 5])
        targets_yaw = torch.sin(y_true[..., 6])
        targets_pitch = torch.cos(y_true[..., 6])
        # targets_pitch = y_true[..., 7]
        targets_roll = y_true[..., 8]

        targets = torch.stack((
            targets_dx, targets_dy, targets_dz,
            targets_dl, targets_dw, targets_dh,
            targets_yaw, targets_pitch, targets_roll
        ), dim=-1)
        targets = targets / torch.tensor([[0.1, 0.1, 0.1, 0.2, 0.2, 0.2, 0.25, 0.25, 0.25]], device=targets.device)
        return targets
